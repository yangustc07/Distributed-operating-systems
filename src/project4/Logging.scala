...
trait Logging extends Actor {
  ...
  private def writeLog(data: String): Unit = getOrElse(x => writeToFile(x.logfileName, data), ())
  private def senderId: Int = getOrElse(_.id, 0)
  private def senderClock: Int = getOrElse(_.lamportClock, 0)
